...
export default function FieldSettingList() {
  const { field } = useParams();
  const location = useLocation();
  const searchParams = new URLSearchParams(location.search);
  const keyFieldsParam = searchParams.get("keyFields");
  const keyFields =
    location.state?.keyFields ||
    (keyFieldsParam ? JSON.parse(decodeURIComponent(keyFieldsParam)) : []);
    ...
    async (currentPage, itemsPerPage, token) => {
      ...
      try {
        ...
        const res = await axios.get(
          `${config.apiUrl}/${field}?${query.toString()}`,
          {
